...
namespace Tools{
  ...
  inline double delta_phi(double phi1, double phi2){
    ...
    return (delta<=TMath::Pi())? delta : 2*TMath::Pi()-delta;
  }
  ...
  // e: 3-vector  Hep3Vector       ..(px,py,pz) or
  //    4-vector  TLorentzVector ..(px,py,pz,E) 
  // Even input the TLorentzVector, we don't use Energy 
  // 
  void EventShape::setPartList(std::vector<CLHEP::HepLorentzVector> e)
  {	
    _myobjects = e;
    ...
    TMatrix temp(3,5);
    Int_t np = 0;
    Int_t numElements = e.size();
    
    for(Int_t elem=0;elem<numElements;elem++) {
      HepLorentzVector o = e.at(elem);
	...
	return;
      ...
      //if (nam.Contains("Hep3Vector")) {
      //	Hep3Vector v(((Hep3Vector *) o)->X(),
      //		   ((Hep3Vector *) o)->Y(),
      //		   ((Hep3Vector *) o)->Z());
      //	mom(np,1) = v.X();
      //	mom(np,2) = v.Y();
      ...
      //	Hep3Vector v(((TLorentzVector *) o)->X(),
      //		   ((TLorentzVector *) o)->Y(),
      //		   ((TLorentzVector *) o)->Z());
      //	mom(np,1) = v.X();
      //	mom(np,2) = v.Y();
      ...
      //}
      //else {
      //	printf("EventShape::setEvent input is not a Hep3Vector or a TLorentzVector\n");
      //	continue;
      //}
      
      Hep3Vector v(o.x(),o.y(),o.z());
  ...
  Hep3Vector EventShape::thrustAxis() {
    Hep3Vector m_ThrustAxis(m_dAxes(1,1),m_dAxes(1,2),m_dAxes(1,3));
    return m_ThrustAxis;
  }
  //______________________________________________________________

  Hep3Vector EventShape::majorAxis() {
    Hep3Vector m_MajorAxis(m_dAxes(2,1),m_dAxes(2,2),m_dAxes(2,3));
    return m_MajorAxis;
  }
  //______________________________________________________________

  Hep3Vector EventShape::minorAxis() {
    Hep3Vector m_MinorAxis(m_dAxes(3,1),m_dAxes(3,2),m_dAxes(3,3));
    return m_MinorAxis;
  }
  //______________________________________________________________
  
  Hep3Vector EventShape::thrust() {
    Hep3Vector m_Thrust(m_dThrust[1],m_dThrust[2],m_dThrust[3]);
    return m_Thrust;
  }
  //______________________________________________________________
  ...
  double EventShape::Acoplanarity()
  {
    if(_myobjects.size() == 2 ) {
      return fabs(TMath::Pi() - delta_phi(_myobjects[0].phi(),_myobjects[1].phi()))
	+ fabs(TMath::Pi() - _myobjects[0].theta() - _myobjects[1].theta());
    }
    return -1;  
  }
