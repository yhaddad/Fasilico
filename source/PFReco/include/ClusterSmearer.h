#ifndef ClusterSmearer_h
#define ClusterSmearer_h 1
 ...
 *
 *  @author F. Gaede, DESY modified H. Yacine
 *  @version $Id: ClusterSmearer.h,v 1.2 2005-10-11 12:56:28 gaede Exp $ 
 */

struct ClusterResolution {
ClusterResolution():A(0.),B(0.),ThMin(0.),ThMax(0.){}
  
ClusterResolution(float a, float b, float thMin, float thMax):A(a),B(b),ThMin(thMin),ThMax(thMax){}
...
class ClusterSmearer : public FourVectorSmearer {
  typedef std::vector<ClusterResolution> ResVec ;
 public:
  ClusterSmearer(const std::vector<float>& resVec ) ;
  
  
  /** Virtual d'tor.*/
  virtual ~ClusterSmearer() {} 
