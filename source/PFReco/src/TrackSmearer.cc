...
TrackSmearer::TrackSmearer(const std::vector<float>& resVec ){
...
HepLorentzVector TrackSmearer::smearedFourVector( const HepLorentzVector& v, int pdgCode ){
  ...
  if( resolution > - 1e-10  ) {
    ...
    n3v.setMag( P + deltaP  ) ;
    //       std::cout << " TrackSmearer::smearedFourVector P0: " 
    // 		<< P << " - P1 : " << P + deltaP 
