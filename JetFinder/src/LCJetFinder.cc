...
void LCJetFinder::processEvent( LCEvent * evt ) { 
  ...
  for (unsigned int i = 0; i < exclusive_jets.size(); i++) {
    ...
    if( exclusive_jets[i].constituents().size()  > 1 ){
...
void LCJetFinder::check( LCEvent * evt ) { 
  ...
  if( jetCol != 0 ){
    ...
    std::vector<int> z_dijet_id = test.Fitter( vec_jets );
    ...
    _roo_dijet.chi2 =  test.chi2fun_zh(dijet, 90.1876, 125, 2.4952, 2.5);
