...
void FastMCProcessor::processEvent( LCEvent * evt ) { 
  ...
  LCCollectionVec* partonVec = new LCCollectionVec( LCIO::RECONSTRUCTEDPARTICLE ) ;
  LCCollectionVec* ISRVec = new LCCollectionVec( LCIO::RECONSTRUCTEDPARTICLE ) ;
  LCRelationNavigator relNav( LCIO::RECONSTRUCTEDPARTICLE , LCIO::MCPARTICLE ) ;
  
  
  std::vector<MCParticle*> mc_partons;
  std::vector<MCParticle*> mc_g;
  std::vector<MCParticle*> ISR_photons;
  
  
  for(int i=0; i<mcpCol->getNumberOfElements() ; i++){
    MCParticle* mcp = dynamic_cast<MCParticle*> ( mcpCol->getElementAt( i ) ) ;
    if( mcp->getParents().size() == 0 ){// parton level :  initial particle
      ReconstructedParticleImpl*  parton = new ReconstructedParticleImpl();
      parton->setMomentum( mcp->getMomentum() );
      parton->setEnergy(mcp->getEnergy());
      parton->setCharge (mcp->getCharge());
      
      if( mcp->getPDG() == 22 ){ 
	ISR_photons.push_back(mcp);
      }
      
      if( fabs( mcp->getPDG()) <= 6  ){ // 
	mc_partons.push_back(mcp);
      }
      
      if( fabs( mcp->getPDG()) == 21  ){ // 
	mc_g.push_back(mcp);
      }
      
      parton->setType(mcp->getPDG());
      partonVec->addElement(parton);
    }
    ...
    if( mcp->getGeneratorStatus() == 1 ) { // stable particles only 
      // tag the ISR photons
      if(mcp->getPDG() == 22 ){ //photon
      	const MCParticleVec gamma_aprents = mcp->getParents();
      	if( gamma_aprents[0]->getPDG()== 22 && gamma_aprents[0]->getParents().size()==0 ){
	  ReconstructedParticleImpl*  isr = new ReconstructedParticleImpl();
	  isr->setMomentum( mcp->getMomentum() );
	  isr->setEnergy(mcp->getEnergy());
	  isr->setCharge (mcp->getCharge());
	  isr->setType(mcp->getPDG());
	  ISRVec->addElement(isr);
  ...
  evt->addCollection( partonVec, "parton");
  evt->addCollection( ISRVec, "ISR");
